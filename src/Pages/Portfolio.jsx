      ...
      <main>
        ...
        <div className = {styles.card_great_container}>
          <ul className={styles.card_list}>
            {workList.map(({id, cover, title, description, link, linkText, mission, problematic}) => ( 
                <li key={id} className={styles.slide_in}> 
                  <Card id = {id} cover = {cover} title = {title} description= {description} link = {link} linkText ={linkText} mission ={mission} problematic= {problematic}/> 
                </li>  
              ))}
            </ul>
